...
select count(distinct a.vdid) as num_vdid_attempt_login
...
,count(distinct(case when dn2.restricted_do_not_copy['input.visitor_device_id'] is not null then a.vdid end)) as num_vdid_reset_pw_login
...
left join dynecom_execution_events dn
	 ...
	 and dn.other_properties['output.mode'] in ('CONFIRMPASSWORDRESETEMAILED','CONFIRMPASSWORDRESETCALLED','CONFIRMPASSWORDRESETTEXTED')
